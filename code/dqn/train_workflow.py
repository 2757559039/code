...
#配置打印类
output_definition = 'output_definition.txt'
definition=CustomPrinter(output_definition)

output_env_info='output_env_info.txt'
op_env_info=CustomPrinter(output_env_info)
...
# 起点为2，终点为1，固定6个宝箱[4, 5, 6, 7, 8, 9]
    ...
    for epoch in range(epoch_num):
        ...
        definition.print(f"这是第{epoch}轮训练")
        op_env_info.print(f"这是第{epoch}轮训练")
...
def run_episodes(n_episode, env, agent, g_data_truncat, usr_conf, logger):
    for episode in range(n_episode):
        definition.print(f"这是第{episode}个周期")
        op_env_info.print(f"这是第{episode}个周期")
        collector = list()
        ...
        while not done:
            ans+=1
            definition.print(f"这是第{ans}步")
            op_env_info.print(f"这是第{ans}步")
            ...
            op_env_info.print(f"这是动作选择{act}")
